...
def make_d2dx2_optt (N):
    ...
    L = np.array([-2,1,1]) # the auxiliary vector (k1) for the left mpo.
    R = np.array([1,1,1])
    op_qtt[0] = ncon ([L,op_qtt[0]], ((1,), (1,-1,-2,-3)))  #L is the vector which contract with k1 leg of op_qttp[0]
...
def project_qtt_op (op_qtt, inds):
    ...
    for n in range(N):
        ...
        if n == 0:
            M = M[inds[n],:,:]      # (ipr, i, k) --> (i,k), the most left mpo, I feed the value of binary array to physical bond of diff_op.
        elif n == N - 1:
            M = M[:,inds[n],:]      # (k, ipr, i) --> (k,i), the most rigit mpo, I feed the value of binary array to physical bond of diff_op.
        else:
            M = M[:,inds[n],:,:]    # (k1,ipr,i,k2) --> (k1,i,k2), the bulk mpo.
        res.append(M)
    return res
...
if __name__ == '__main__':
    ...
    xsq_qtt = xsq.make_xsq_qtt (N, factor)  # A sine function in QTT form
    ...
    for indices in itertools.product(*ranges):
        ...
        op_qtt_proj = project_qtt_op (op_qtt, inds)
        f = xsq.get_ele (xsq_qtt, inds)
        d2f = contract_qtt (op_qtt_proj, xsq_qtt)
